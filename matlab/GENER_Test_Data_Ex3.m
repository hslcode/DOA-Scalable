...
for i=1:array_num
    ...
    R_sam_d1 = zeros(ULA_N,ULA_N,3,length(DOA_set),Test_mum);
    R_sam_d2 = zeros(ULA_N,ULA_N,3,length(DOA_set),Test_mum);
    ...
    for k = 1:Test_mum
        ...
        for j =1:length(DOA_set)
            [sam,~] = Gener_Sam_Covar_Matrix(DOA_set(j),T,SNR_dB,ULA_N,d_ref);
            R_sam_d1(:,:,:,j,k)=sam;
            [sam,~] = Gener_Sam_Covar_Matrix(DOA_set(j),T,SNR_dB,ULA_N,d);
            R_sam_d2(:,:,:,j,k)=sam;
        end
    end

    filename = fullfile('E:\code\array_tansfor\data\EX3',sprintf("EX1_Alpha_%s.h5", alpha_str(i)));
    h5create(filename,'/sam_d1', size(R_sam_d1));
    h5write(filename, '/sam_d1', R_sam_d1);
    h5create(filename,'/sam_d2', size(R_sam_d2));
    h5write(filename, '/sam_d2', R_sam_d2);
    h5create(filename,'/angle', size(DOA_ture));
    h5write(filename, '/angle', DOA_ture);
end
