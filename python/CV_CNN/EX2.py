...
for doa_index in range(DOA_num_d1):
        ...
        x_pred_sam = model(X_test_data_sam_d1[doa_index,:,:,:]).detach().cpu().numpy()
        doa_d1 = np.argmax(x_pred_sam) - 60
        est_DOA_d1.append(doa_d1)


        x_pred_sam = model(X_test_data_sam_d2[doa_index,:,:,:]).detach().cpu().numpy()
...
RMSE_d1 = np.sqrt(mean_squared_error(est_DOA_d1, GT_angles_d1))
RMSE_d2 = np.sqrt(mean_squared_error(est_DOA_d2, GT_angles_d1))
print(RMSE_d1)
print(RMSE_d2)
plt.figure(1)
