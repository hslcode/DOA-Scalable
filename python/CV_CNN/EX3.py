...
for alpha in alpha_set: #Traverse the alpha set
    ...
    X_test_data_sam_gamma_base = (RX_sam_test_gamma_base[:, :, 0, :, :].type(torch.complex64) +
                       1j * RX_sam_test_gamma_base[:, :, 1, :, :].type(torch.complex64)).unsqueeze(2).to(device)
    X_test_data_sam_gamma_bias = (RX_sam_test_gamma_bias[:, :, 0, :, :].type(torch.complex64) +
                       1j * RX_sam_test_gamma_bias[:, :, 1, :, :].type(torch.complex64)).unsqueeze(2).to(device)
    ...
    for doa_index in range(DOA_num):    #Traverse the EFOV
        ...
        for test_index in np.arange(0,test_num,test_batch): # Monte Carlo test
            x_pred_sam = model_CV_CNN(X_test_data_sam_gamma_base[test_index:test_index+test_batch, doa_index,:,:,:]).detach().cpu().numpy()
            doa_base = np.argmax(x_pred_sam,axis=-1) - 60  #The base estimation i.e. \theta_{base}
            est_DOA_gamma_base.append(doa_base)

            x_pred_sam = model_CV_CNN(X_test_data_sam_gamma_bias[test_index:test_index+test_batch, doa_index,:,:,:]).detach().cpu().numpy()
            doa_bias = np.argmax(x_pred_sam,axis=-1) - 60  #The bias estimation i.e. \theta_{bias}
        ...
        RMSE_gamma_base.append(
            np.sqrt(mean_squared_error(est_DOA_gamma_base, np.repeat(GT_angles[doa_index], test_num, axis=0))))
        
        #The RMSE of generalized estimation
        RMSE_gamma_bias.append(
            np.sqrt(mean_squared_error(est_DOA_gamma_bias, np.repeat(GT_angles[doa_index], test_num, axis=0))))
...
with h5py.File(f_result, 'a') as hf:  
        if 'Ave_RSME_gamma_base_CVCNN' in hf:
                del hf['Ave_RSME_gamma_base_CVCNN']
        if 'Ave_RSME_gamma_bias_CVCNN' in hf:
                del hf['Ave_RSME_gamma_bias_CVCNN']
        if 'Ave_RSME_ratio_CVCNN' in hf:
            del hf['Ave_RSME_ratio_CVCNN']
        hf.create_dataset('Ave_RSME_gamma_base_CVCNN', data=Ave_RSME_gamma_base)
        hf.create_dataset('Ave_RSME_gamma_bias_CVCNN', data=Ave_RSME_gamma_bias)
        hf.create_dataset('Ave_RSME_ratio_CVCNN', data=Ave_RSME_ratio)
        hf.close()