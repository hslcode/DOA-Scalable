...
for alpha in alpha_set: #Traverse the alpha set
    ...
    X_test_data_sam_gamma_base = RX_sam_test_gamma_base.swapaxes(2, 4)
    RX_sam_test_gamma_bias = np.array(f2['sam_gamma_bias'])
    X_test_data_sam_gamma_bias = RX_sam_test_gamma_bias.swapaxes(2, 4)
    ...
    for doa_index in range(DOA_num): #Traverse the EFOV
        ...
        for test_index in np.arange(0,test_num,test_batch): # Monte Carlo test
            ...
            x_pred_sam = model_CNN.predict(X_test_data_sam_gamma_base[test_index:test_index+test_batch, doa_index,:,:,:],verbose=0)
            doa_base = np.argmax(x_pred_sam,axis=-1) - 60  #The base estimation i.e. \theta_{base}
            est_DOA_gamma_base.append(doa_base)

            x_pred_sam = model_CNN.predict(X_test_data_sam_gamma_bias[test_index:test_index+test_batch, doa_index,:,:,:],verbose=0)
...
with h5py.File(f_result, 'a') as hf:  
        if 'Ave_RSME_gamma_base_2DCNN' in hf:
                del hf['Ave_RSME_gamma_base_2DCNN']
        if 'Ave_RSME_gamma_bias_2DCNN' in hf:
                del hf['Ave_RSME_gamma_bias_2DCNN']
        if 'Ave_RSME_ratio_2DCNN' in hf:
            del hf['Ave_RSME_ratio_2DCNN']
        hf.create_dataset('Ave_RSME_gamma_base_2DCNN', data=Ave_RSME_gamma_base)
        hf.create_dataset('Ave_RSME_gamma_bias_2DCNN', data=Ave_RSME_gamma_bias)
        hf.create_dataset('Ave_RSME_ratio_2DCNN', data=Ave_RSME_ratio)
        hf.close()