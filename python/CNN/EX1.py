...
X_test_data_sam = RX_sam_test.swapaxes(2,4)
...
for doa_index in range(DOA_num):        #Traverse the EFOV
        for test_index in range(test_num):      # Monte Carlo test
                x_pred_sam = model_CNN.predict(np.expand_dims(X_test_data_sam[test_index,doa_index,:,:,:],0))
...
with h5py.File(f_result, 'a') as hf: 
        if 'Est_phase_2D_CNN' in hf:
                del hf['Est_phase_2D_CNN']
        if 'Ture_phase_2D_CNN' in hf:
                del hf['Ture_phase_2D_CNN']
        hf.create_dataset('Est_phase_2D_CNN', data=Est_phase_average)
        hf.create_dataset('Ture_phase_2D_CNN', data=Ture_phase[:,0])
        hf.close()
